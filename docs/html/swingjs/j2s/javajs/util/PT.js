...
C$.$fields$=[[]
,['O',['tensScale','float[]','+decimalScale','decimalScaleD','double[]']]]
...
Clazz.newMeth(C$, 'approxD$D$F',  function (f, n) {
return Math.round$D(f * n) / n;
}, 1);
...
if (!Float.isNaN$F(floatT)) strLabel+=C$.formatF$F$I$I$Z$Z(floatT, width, (st.equals$O("f") || st.equals$O("p")  ? precision : -1 - precision), alignLeft, zeroPad);
...
Clazz.newMeth(C$, 'fixPtFloats$javajs_util_T3$D',  function (pt, d) {
pt.x=(Math.round$D(pt.x * d) / d);
pt.y=(Math.round$D(pt.y * d) / d);
pt.z=(Math.round$D(pt.z * d) / d);
}, 1);
...
Clazz.newMeth(C$, 'fixPtDoubles$javajs_util_T3d$D',  function (pt, f) {
